...
public class SoundUtils {
	...
	public static void beep(double freq, final double millis) throws InterruptedException, LineUnavailableException {
		...
		try{/*
			 ...
			 */
			if (af.getEncoding() != AudioFormat.Encoding.PCM_SIGNED) throw new UnsupportedOperationException("Unknown encoding");

			if (af.getSampleSizeInBits() != 16) throw new UnsupportedOperationException("Weird sample size");
			...
			for (int frame = 0; frame < frames; frame++) {
				short sample = (short)(ampFactor * Math.sin(frame * freqFactor));
